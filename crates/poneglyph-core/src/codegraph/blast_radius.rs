...
pub fn blast_radius(store: &Store, target: &str, max_depth: usize) -> Result<BlastRadiusReport> {
    let by_file = store.cg_nodes_in_file(target)?;
    let root = if !by_file.is_empty() {
        by_file
    } else {
        store.cg_nodes_by_name(target, &[CgNodeKind::Function, CgNodeKind::Method, CgNodeKind::Type])?
    };
    if root.is_empty() {
    ...
    while depth < max_depth && !frontier.is_empty() {
        ...
        for node in &frontier {
            let mut callers = store.cg_edges_into(&node.id, CgEdgeKind::Calls)?;
            callers.extend(store.cg_edges_into(&node.id, CgEdgeKind::Imports)?);
    ...
    for id in visited.keys() {
        for test in store.cg_edges_into(id, CgEdgeKind::Tests)? {
...
mod tests {
    ...
    fn traces_transitive_callers_bounded_by_depth() {
        ...
        let report = blast_radius(&store, "a", 5).unwrap();
        ...
        let shallow = blast_radius(&store, "a", 1).unwrap();
    ...
    fn includes_covering_tests() {
        ...
        let report = blast_radius(&store, "add", 5).unwrap();
    ...
    fn unknown_target_returns_empty() {
        let store = Store::open_in_memory().unwrap();
        let report = blast_radius(&store, "nonexistent", 5).unwrap();
    ...
    fn file_target_uses_all_symbols_in_file_as_roots() {
        ...
        let report = blast_radius(&store, "lib.rs", 5).unwrap();
